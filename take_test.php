...
<?php $query1 = mysqli_query($link, "select * from student_class_quiz where student_id = '$session_id' and class_quiz_id = '$class_quiz_id' ") or die(mysqli_error($link));
$count = mysqli_num_rows($query1);
?>

<?php
if ($count > 0) { } else {
	mysqli_query($link, "insert into student_class_quiz (class_quiz_id,student_id,student_quiz_time) values('$class_quiz_id','$session_id','$quiz_time')");
}
?>
				...
				<div class="row-fluid">
					...
					<div id="block_bg" class="block">
						<div class="navbar navbar-inner block-header">
							...
							<?php
							if ($_GET['test'] == 'ok') {
								$sqlp = mysqli_query($link, "SELECT * FROM class_quiz WHERE class_quiz_id = '$class_quiz_id'") or die(mysqli_error($link));
								...
								<form action="take_test.php<?php echo '?id=' . $get_id; ?>&<?php echo 'class_quiz_id=' . $class_quiz_id; ?>&<?php echo 'test=done' ?>&<?php echo 'quiz_id=' . $quiz_id; ?>&<?php echo 'quiz_time=' . $quiz_time; ?>" name="testform" method="POST" id="test-form">
									<?php
									$sqla = mysqli_query($link, "select * FROM class_quiz 
										LEFT JOIN quiz ON quiz.quiz_id  = class_quiz.quiz_id
										where teacher_class_id = '$get_id' 
										order by date_added DESC ") or die(mysqli_error($link));
									$rowa = mysqli_fetch_array($sqla);

									?>
									<h3>Name: <b><?php echo $rowa['quiz_title']; ?></b></h3>
									...
									<table class="questions-table table">
										...
										<?php
										$sqlw = mysqli_query($link, "SELECT * FROM quiz_question where quiz_id = '$quiz_id'  ORDER BY RAND()");
										...
										while ($roww = mysqli_fetch_array($sqlw)) {
											...
											<tr id="q_<?php echo $x = $x + 1; ?>" class="questions">
												...
												<td id="qa">
													...
													$sqly = mysqli_query($link, "SELECT * FROM answer WHERE quiz_question_id = '" . $roww['quiz_question_id'] . "'");
												...
												?>
													<br />
													<button onclick="return false;" qn="<?php echo $x; ?>" class="nextq btn btn-info" id="next_<?php echo $x; ?>">Next question <i class="icon-arrow-right"></i> </button>
									...
									?>
										<tr>
											<td></td>
											<td>
												<button class="btn btn-info" id="submit-test" name="submit_answer"><i class="icon-check"></i> Answer</button>
											</td>
										</tr>
									</table>
							...
							for ($x = 1; $x <= $x1; $x++) {
								...
								$sql = mysqli_query($link, "SELECT * FROM quiz_question WHERE quiz_question_id = " . $x2 . "");
								...
								mysqli_query($link, "UPDATE student_class_quiz SET `student_quiz_time` = 3600, `grade` = '" . $score . " out of " . ($x - 1) . "' WHERE student_id = '$session_id' and class_quiz_id = '$class_quiz_id'") or die(mysqli_error($link));
								?>
								<script>
