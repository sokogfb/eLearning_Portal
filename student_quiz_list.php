                ...
                <div class="span9" id="content">
                     <div class="row-fluid">
                        ...
                        <div id="block_bg" class="block">
							...
							<?php 		$query = mysqli_query($link, "select * FROM class_quiz 
                                ...
                                <div class="span12">
									...
									<?php include('copy_to_backpack_modal.php'); ?>
  									<table cellpadding="0" cellspacing="0" class="table" id="">
										...
										<tbody>
                              		<?php
										$query = mysqli_query($link, "select * FROM class_quiz 
										LEFT JOIN quiz on class_quiz.quiz_id = quiz.quiz_id
										where teacher_class_id = '$get_id'  order by class_quiz_id DESC ")or die(mysqli_error($link));
										while($row = mysqli_fetch_array($query)){
										$id  = $row['class_quiz_id'];
										$quiz_id  = $row['quiz_id'];
										$quiz_time  = $row['quiz_time'];
									
										$query1 = mysqli_query($link, "select * from student_class_quiz where class_quiz_id = '$id' and student_id = '$session_id'")or die(mysqli_error($link));
									...
									?>                              
										<tr>                     
                                         ...
                                         <td width="200">
										<?php if ($grade == ""){ ?>
											<a  data-placement="bottom" title="Take This Quiz" id="<?php echo $id; ?>Download" href="take_test.php<?php echo '?id='.$get_id ?>&<?php echo 'class_quiz_id='.$id; ?>&<?php echo 'test=ok' ?>&<?php echo 'quiz_id='.$quiz_id; ?>&<?php echo 'quiz_time='.$quiz_time;	 ?>"><i class="icon-check icon-large"></i> Start the quiz</a>
										<?php }else{ ?>
										<b>Points <?php echo $grade; ?></b>
										<?php } ?>
										</td>            
