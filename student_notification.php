							...
							<div class="span12">
								...
								<form action="read.php" method="post">
									<?php if ($not_read == '0') { } else { ?>
										<button id="delete" class="btn btn-info" name="read"><i class="icon-check"></i> Mark as read</button>
									...
									<?php $query = mysqli_query($link, "select * from teacher_class_student
					...
					") or die(mysqli_error($link));
									...
									if ($count  > 0) {
										while ($row = mysqli_fetch_array($query)) {
											...
											$query_yes_read = mysqli_query($link, "select * from notification_read where notification_id = '$id' and student_id = '$session_id'") or die(mysqli_error($link));
									...
									?>
										<div class="alert alert-info"><strong><i class="icon-info-sign"></i> There are no notifications to show.</strong></div>
									<?php
								}
								?>
