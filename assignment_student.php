				...
				<div class="row-fluid">
					...
					<div id="block_bg" class="block">
						<div class="navbar navbar-inner block-header">
							<?php $query = mysqli_query($link, "select * FROM assignment where class_id = '$get_id'  order by fdatein DESC") or die(mysqli_error($link));
						...
						<div class="block-content collapse in">
							<div class="span12">
								<?php
								$query = mysqli_query($link, "select * FROM assignment where class_id = '$get_id'  order by fdatein DESC") or die(mysqli_error($link));
								$count = mysqli_num_rows($query);
								if ($count == '0') { ?>
									<div class="alert alert-info">There are no assignments to show.</div>
								<?php
							} else {
								?>
												...
												<tr>
													...
													<td width="220">
														<form id="assign" method="post" action="submit_assignment.php<?php echo '?id=' . $get_id ?>&<?php echo 'post_id=' . $id ?>">
															...
															if ($floc == "") { } else {
																?>
																<a data-placement="bottom" title="Download" id="<?php echo $id; ?>download" class="btn btn-info" href="<?php echo $row['floc']; ?>"><i class="icon-download icon-large"></i> Download</a>
															<?php } ?>
															<button data-placement="bottom" title="Submit " id="<?php echo $id; ?>submit" class="btn btn-info" name="btn_assign"><i class="icon-upload icon-large"></i> Submit</button>
														</form>
													</td>
