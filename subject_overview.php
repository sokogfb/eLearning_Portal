						...
						<div class="navbar navbar-inner block-header">
							<div id="" class="muted pull-right">
								<?php $query = mysqli_query($link, "select * from teacher_class
										...
										where class_subject_overview.teacher_class_id = '$get_id'") or die(mysqli_error($link));
								$row = mysqli_fetch_array($query);
								$id = $row['class_subject_overview_id'];
								$count = mysqli_num_rows($query);
								if ($count > 0) {
									?>
									<a href="edit_subject_overview.php<?php echo '?id=' . $get_id; ?>&<?php echo 'subject_id=' . $id; ?>" class="btn btn-info"><i class="icon-pencil"></i> Edit</a>
								<?php } else { ?>
									<a href="add_subject_overview.php<?php echo '?id=' . $get_id; ?>" class="btn btn-info"><i class="icon-plus-sign"></i> Add</a>
								<?php } ?>
							</div>
						</div>
